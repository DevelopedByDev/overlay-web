# Agent setup for `overlay-landing`

This is a deepsec scanning workspace. Project `overlay-landing` was just registered
(target: `..`). Setup is incomplete — `data/overlay-landing/INFO.md`
...
2. **Fill in `data/overlay-landing/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id overlay-landing
pnpm deepsec process --project-id overlay-landing
```
