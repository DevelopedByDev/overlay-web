...
Currently configured project: `overlay-landing` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/overlay-landing/SETUP.md` to fill in
   `data/overlay-landing/INFO.md`.
...
`--project-id overlay-landing` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/overlay-landing/`.
