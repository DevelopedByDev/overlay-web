          ...
          >
            ...
            <div className="p-4 space-y-1">
              <div className="flex items-start gap-3">
                <p className="min-w-0 flex-1 text-sm text-[#0a0a0a] line-clamp-2">{outputLabel(lightbox)}</p>
                <button
                  type="button"
...
function OutputCard({ output, onExpand, onDetails }: { output: Output; onExpand: () => void; onDetails: () => void }) {
  ...
  return (
    <div
      className="mb-4 block w-full break-inside-avoid overflow-hidden rounded-xl border border-[#e5e5e5] bg-white group cursor-pointer hover:shadow-md transition-shadow"
    ...
    >
      <div className="relative bg-[#f5f5f5]">
        {isCompleted && output.url && output.type === 'image' && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={output.url} alt={outputLabel(output)} className="block w-full h-auto max-h-[22rem] rounded-t-xl object-cover" />
        )}
      ...
      <div className="px-3 py-2">
        <div className="flex items-start gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-xs text-[#525252] line-clamp-2 leading-relaxed">{outputLabel(output)}</p>
            {!isMedia && <p className="mt-1 text-[11px] text-[#888] line-clamp-2">{output.prompt}</p>}
