...
export default function NotebookEditor({
  ...
  function patchActiveNoteMetadata(patch: Partial<Pick<Note, 'icon' | 'coverImage' | 'coverPosition'>>) {
    if (!activeNote) return
    const nextNote = { ...activeNote, ...patch }
    setActiveNote(nextNote)
    setNotes((prev) => prev.map((note) => (note._id === nextNote._id ? nextNote : note)))
    void fetch('/api/app/notes', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ noteId: activeNote._id, ...patch }),
    }).catch(() => {})
  }

  function handleCoverChange(file: File) {
    const reader = new FileReader()
    reader.onload = () => patchActiveNoteMetadata({ coverImage: String(reader.result), coverPosition: 0.5 })
          ...
          <>
            <EditorShell
              ...
              noteMeta={{
                ...
                coverImage: activeNote.coverImage,
                coverPosition: activeNote.coverPosition,
              }}
              aiOpen={aiOpen}
              onAiOpen={() => setAiOpen(true)}
              onAiClose={() => setAiOpen(false)}
              onTitleChange={handleTitleChange}
              onIconChange={(icon) => patchActiveNoteMetadata({ icon })}
              onCoverChange={handleCoverChange}
              onCoverRemove={() => patchActiveNoteMetadata({ coverImage: undefined, coverPosition: undefined })}
            />
            <SlashMenu
