...
import { DraftReviewModal, FlashCopyIconButton } from './Modals'
import { AutomationEditorPanel, type AutomationDetail, type AutomationDetailTab, normalizeAutomationDetailTab, AUTOMATION_DETAIL_TABS } from './AutomationEditor'
import {
...
export default function ChatInterface({
  ...
  const handleStartTopUp = useCallback(async () => {
    ...
    try {
      const response = await overlayAppClient.topUps.checkoutResponse({
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void overlayAppClient.topUps.verifyResponse({ sessionId: topUpSessionId })
      .then(async (response) => {
        const data = await response.json().catch(() => ({}))
        if (cancelled) return
        if (response.ok) {
