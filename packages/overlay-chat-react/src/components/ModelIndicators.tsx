...
export interface ChatModelIndicatorInfo {
  cost: number
  supportsVision: boolean
  supportsReasoning: boolean
}

export interface ChatModelQualityInfo extends ChatModelIndicatorInfo {
  intelligence: number
...
export function ModelQualitiesPanel({ model }: { model: ChatModelQualityInfo | null | undefined }) {
  ...
  return (
    <div className="pointer-events-none flex flex-col gap-1">
      <MetricRow
        icon={BrainCircuit}
        label="Intelligence"
        value={Math.round(model.intelligence)}
      />
      <MetricRow
        icon={DollarSign}
        label="Cost"
