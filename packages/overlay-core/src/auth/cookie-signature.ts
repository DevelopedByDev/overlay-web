// @overlay/core — extracted from src/lib/session-cookie-signature.ts
// HMAC-SHA256 cookie signature validation. Uses Web Crypto API.
...
async function getSigningKey(): Promise<CryptoKey | null> {
  if (!signingKeyPromise) {
    const secret = (typeof process !== 'undefined' ? process.env.SESSION_SECRET : undefined)?.trim()
