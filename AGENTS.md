...
- App code is layered: `src/features/<domain>/`, `src/components/{ui,layout,providers}/`, `src/server/` (server-only), and `src/shared/` (isomorphic). Reusable contracts and adapters live in workspace `packages/` (`@overlay/app-core`, `auth-contracts`, `storage-contracts`, `llm-gateway`, `agent-runtime`, `tools-core`, `api-client`, etc.). Legacy `src/lib/` is gone. tsconfig aliases: `@/server/*`, `@/shared/*`, `@/features/*`.
