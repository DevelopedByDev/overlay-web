...
- App code is layered: `src/features/<domain>/` (domain UI + feature `lib/`), `src/components/{ui,layout,providers}/` (shared chrome), `src/server/` (Next/API server logic; every `.ts` file has `import 'server-only'`), and `src/shared/` (isomorphic). Legacy `src/lib/` is gone. tsconfig aliases: `@/server/*`, `@/shared/*`, `@/features/*`.
